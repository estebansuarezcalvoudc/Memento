...
from app.services.conversation.rag_service import RagService
...
def reset_conversation_singletons():
    """
    Reset ConversationService and RagService singletons before and after each
    ...
    SingletonMeta._instances.pop(RagService, None)
    yield
    SingletonMeta._instances.pop(ConversationService, None)
    SingletonMeta._instances.pop(RagService, None)
...
def mock_rag_get_reply():
    """
    Mock RagService.get_reply to avoid real LLM and ChromaDB calls.
    Returns a canned AI response.
    """
    with patch(
        "app.services.conversation.rag_service.RagService.get_reply",
